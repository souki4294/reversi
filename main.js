...
const changeTurn = () => {
  ...
  if (currentColor === 1) {
    currentTurnText.textContent = "ジバニャン";
  } else {
    currentTurnText.textContent = "コマさん";
  }
}
...
const onClickSquare = (index) => {
  ...
  if (stoneStateList.every((state) => state !== 0)) {
    ...
    if (blackStonesNum > whiteStonesNum) {
      winnerText = "ジバニャンの勝ちです！";
    } else if (blackStonesNum < whiteStonesNum) {
      winnerText = "コマさんの勝ちです！";
    } else {
      winnerText = "コマじろうの勝ちです";
    }

    alert(`ゲーム終了です。白${whiteStonesNum}、赤${blackStonesNum}で、${winnerText}`)
  }
